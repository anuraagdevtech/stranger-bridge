...
//
// react-native-webrtc requires a custom Expo dev-client build (it ships
// native modules Expo Go doesn't include) — see README for `expo run:ios`
// / `expo run:android` instructions.
...
function offerHasVideo(data: unknown): boolean {
  const sdp = (data as RTCSessionDescription | undefined)?.sdp ?? "";
  return sdp.includes("m=video");
}

/** Mobile counterpart to apps/web's useWebRtc — same signaling protocol, react-native-webrtc under the hood. */
export function useWebRtc(roomId: string, peerId: string, selfId: string | undefined) {
  ...
  const createPeerConnection = useCallback(() => {
    ...
    // @ts-expect-error react-native-webrtc event typing differs slightly from lib.dom
    pc.onicecandidate = (e) => {
  ...
  useEffect(() => {
    ...
    const onOffer = (signal: WebRtcSignal) => {
      ...
      pendingOfferRef.current = signal.data as { type: string; sdp: string };
    ...
    const onAnswer = async (signal: WebRtcSignal) => {
      ...
      await pcRef.current.setRemoteDescription(
        new RTCSessionDescription(signal.data as { type: string; sdp: string }),
      );
